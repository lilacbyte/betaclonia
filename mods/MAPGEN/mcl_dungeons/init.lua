...
local min_y_nether = math.max(mcl_vars.mg_nether_min, mcl_vars.mg_bedrock_nether_bottom_max) + 1
local max_y_nether = math.min(mcl_vars.mg_nether_max - 1, mcl_vars.mg_bedrock_nether_top_min - 1)
-- Calculate the number of dungeon spawn attempts
-- In The OG Game, there 8 dungeon spawn attempts The OG Game chunk (16*256*16 = 65536 blocks).
-- Minetest chunks don't have this size, so scale the number accordingly.
-- Performance: keep dungeons intentionally rare for beta-style gameplay.
-- Old value was ~63 attempts per generated 80x80x80 chunk, which is expensive.
local attempts = 6
...
{
	{
		...
		items = {
			...
			{ itemstring = "mcl_jukebox:record_2", weight = 10 },
			{ itemstring = "mcl_jukebox:record_3", weight = 10 },
			...
			{ itemstring = "mcl_core:apple", weight = 8, amount_min = 1, amount_max = 1 },
		}
	},
	{
		stacks_min = 1,
		...
		items = {
			...
			{ itemstring = "mcl_farming:potato_item", weight = 10, amount_min = 1, amount_max = 4 },
			{ itemstring = "mcl_trees:sapling_mangrove", weight = 2 },
			{ itemstring = "mcl_trees:sapling_cherry_blossom", weight = 1 },
			{ itemstring = "mcl_trees:sapling_pale_oak", weight = 1 },
	...
	},
	{
		...
		items = {
			...
			{ itemstring = "mcl_mobitems:gunpowder", weight = 10, amount_min = 1, amount_max = 8 },
				{ itemstring = "mcl_mobitems:feather", weight = 10, amount_min = 1, amount_max = 8 },
...
local function ecb_spawn_dungeon(_, _, calls_remaining, param)
	...
	local realm = param.realm or "overworld"
	local loottable = realm == "nether" and loottable_nether or loottable_overworld
	local floor_node = realm == "nether" and "mcl_nether:nether_brick" or "mcl_core:mossycobble"
	local floor_node_alt = realm == "nether" and "mcl_nether:cracked_nether_brick" or "mcl_core:cobble"
	local wall_node = realm == "nether" and "mcl_nether:nether_brick" or "mcl_core:cobble"
		...
		if rn and rn.is_ground_content or name == "mcl_core:cobble" or name == "mcl_core:mossycobble" then
			...
			if ty == y then
				if pr:next(1,4) == 1 then
					minetest.swap_node(p, {name = floor_node_alt})
				else
					minetest.swap_node(p, {name = floor_node})
				end
			...
			elseif tx == x or tz == z or tx == maxx or tz == maxz then
				...
				if (ty == maxy) or (not (openings[tx] and openings[tx][tz]))  then
					...
					minetest.swap_node(p, {name = wall_node})
	...
	for s=#spawner_posses, 1, -1 do
		...
		local spawner_mob
		if realm == "nether" then
			spawner_mob = "mobs_mc:witherskeleton"
		else
			local mobs = {
				"mobs_mc:zombie",
				"mobs_mc:zombie",
			...
			spawner_mob = mobs[pr:next(1, #mobs)]
		end
...
local function dungeons_nodes(minp, maxp, blockseed)
	...
	-- Further reduce per-chunk overhead: only process ~1 out of 4 chunks.
	if pr:next(1, 4) ~= 1 then
		return false
	end
	local function spawn_realm_dungeons(ymin, ymax, realm)
		if ymax < ymin then return end
		for _ = 1, attempts do
			...
			if ymin <= ymax - dim.y - 1 then
				...
				minetest.emerge_area(p1, p2, ecb_spawn_dungeon, {p1=p1, p2=p2, dim=dim, pr=pr, realm=realm})
			end
		end
	end
	spawn_realm_dungeons(math.max(min_y, minp.y), math.min(max_y, maxp.y), "overworld")
	spawn_realm_dungeons(math.max(min_y_nether, minp.y), math.min(max_y_nether, maxp.y), "nether")
end
...
function mcl_dungeons.spawn_dungeon(p1, _, pr)
	...
	minetest.log("verbose","[mcl_dungeons] size=" ..minetest.pos_to_string(dim) .. ", emerge from "..minetest.pos_to_string(p1) .. " to " .. minetest.pos_to_string(p2))
	minetest.emerge_area(p1, p2, ecb_spawn_dungeon, {p1=p1, p2=p2, dim=dim, pr=pr, dontcheck=true, realm="overworld"})
end
