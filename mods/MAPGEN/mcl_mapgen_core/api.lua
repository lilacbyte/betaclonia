...
function minetest.register_on_generated(node_function)
	local localothernumber = minetest.get_current_modname()
	local localthisnumber = #registered_generators
	if localthisnumber ~= nil then
		if localothernumber ~= nil then
			mcl_mapgen_core.register_generator("mod_"..localothernumber.."_"..tostring(localthisnumber+1), nil, node_function)
		end
	end
end
