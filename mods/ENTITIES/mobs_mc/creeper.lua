...
local creeper_defs = {
	...
	on_attack = function() end,
}

if minetest.get_modpath("mcl_jukebox") then
	creeper_defs.on_die = function(_, pos, cmi_cause)
		-- Drop a random music disc when killed by skeleton
		if cmi_cause and cmi_cause.type == "arrow" then
			if cmi_cause.mob_name == "mobs_mc:skeleton" then
