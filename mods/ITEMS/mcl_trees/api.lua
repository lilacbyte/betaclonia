...
local enable_trapdoors = false
local enable_fence_gates = false
...
--	_on_axe_place = mcl_trees.strip_tree,--removed
...
if minetest.get_modpath("screwdriver") then
	tpl_log.on_rotate = screwdriver.rotate_3way
end
local tpl_wood = {
	_doc_items_hidden = false,
...
--			_mcl_stripped_variant = (p.stripped == nil or type(p.stripped) == "table") and "mcl_trees:stripped_"..name,--removed
		},p.tree or {})
	...
	if p.bark == nil or type(p.bark) == "table" then
		local def = table.merge(tpl_log, {
			...
			is_ground_content = false,
--			_mcl_stripped_variant = (p.stripped_bark == nil or type(p.stripped_bark) == "table") and "mcl_trees:bark_stripped_"..name,--removed
		}, p.bark or {})
...
--[[	if p.stripped_bark == nil or type(p.stripped_bark) == "table" then
	...
	if enable_fence_gates and (p.fence_gate == nil or type(p.fence_gate) == "table") then
	...
	if enable_trapdoors and (p.trapdoor == nil or type(p.trapdoor) == "table") then
...
--				def.description = def.description or D(rname .. " Bark Slab")--removed
--				mcl_stairs.register_slab(name.."_bark", def)--removed
			end
		end
	end
