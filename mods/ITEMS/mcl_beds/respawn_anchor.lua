...
for i=0,4 do
	...
	local function rightclick(pos, node, player, itemstack)
		...
		elseif mcl_worlds.pos_to_dimension(pos) ~= "nether" then
			if node.name ~= "mcl_beds:respawn_anchor" then --only charged respawn anchors are exploding in the overworld & end in The OG Game
	...
	if i == 0 then
		minetest.register_node("mcl_beds:respawn_anchor",{
			...
			groups = {pickaxey=1, material_stone=1, deco_block=1, not_in_creative_inventory = 1},--added "not in creative inventory", the functionality can be retained for the likes of adventure maps etc since it's not too bad of a function per se, but it will be hidden and be unobtainable without the use of commands
			_mcl_hardness = 22.5,
