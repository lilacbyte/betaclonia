...
minetest.register_on_mods_loaded(function()
	...
	for name, def in pairs(minetest.registered_items) do
		...
		if def._get_all_virtual_items then
			for category, list in pairs(def._get_all_virtual_items()) do
				local cat = category == "brew" and "matr" or category
				for _, virtual_item in pairs(list) do
					if inventory_lists[cat] then
						table.insert(inventory_lists[cat], virtual_item)
					end
	...
	for key, to_sort in pairs(inventory_lists) do
		local deduped = dedupe_list(to_sort)
		table.sort(deduped)
		inventory_lists[key] = deduped
	end
end)
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	...
	if players[name].page == "brew" then
		players[name].page = "matr"
	end
