...
local function get_mouse_button(player)
	...
	if controls.RMB and minetest.get_item_group(get_wielded_item_name, "bow") == 0 and
		minetest.get_item_group(get_wielded_item_name, "crossbow") == 0 or controls.LMB then--removed and not mcl_shields.wielding_shield(player, 1) and not mcl_shields.wielding_shield(player, 2)
		return true
	else
		return false
	end
...
mcl_player.register_globalstep(function(player)
	...
	if parent or mcl_player.players[player].attached then
		...
		elseif walking and (math.abs(velocity.x) > 0.35 or math.abs(velocity.z) > 0.35) then --walking
			...
			local no_arm_moving = minetest.get_item_group(wielded_itemname, "bow") > 0--removed or minetest.get_item_group(wielded_itemname, "crossbow") > 0 or mcl_shields.wielding_shield(player, 1) or mcl_shields.wielding_shield(player, 2)
			...
			if no_arm_moving and control.RMB and control.sneak or minetest.get_item_group(wielded_itemname, "crossbow") > 0 and control.sneak then
			...
			elseif is_sprinting and not control.sneak then
...
--[[	if mcl_shields.is_blocking(player) == 2 then
	...
	if minetest.get_item_group(wielded_itemname, "crossbow") == 5 then
...
mcl_player.register_globalstep_slow(function(player)
	-- Ensure old underwater particle spawners are cleaned up.
	if mcl_player.players[player].pspawner_underwater then
